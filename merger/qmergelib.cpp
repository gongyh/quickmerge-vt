...
void writeToFile(asmMerge &merge, string &prefix) {
  ...
  for (unsigned int i = 0; i < merge.r_name.size(); i++) {
    ...
    for (unsigned int j = 0; j < merge.ref_st[tempname].size(); j++) {
      fout << merge.r_name[i] << '\t' << merge.q_name[i] << '\t'
           << merge.ref_len[tempname] << '\t' << merge.q_len[tempname] << '\t'
           << merge.ref_st[tempname][j] << '\t' << merge.ref_end[tempname][j]
           << '\t' << merge.q_st[tempname][j] << '\t'
           << merge.q_end[tempname][j] << '\t' << endl;
    }
  }
  fout.close();
}
...
/////////////////////////////////////////////////////////////////////////this
///function checks whether a query is completely contained within a reference
...
void innieChecker(asmMerge &merge) {
  ...
  for (unsigned int i = 0; i < merge.r_name.size(); i++) {
    ...
    if (merge.newEnd[tempname][2] < merge.newEnd[tempname][3]) {
      if (((merge.newEnd[tempname][0] - 1) >=
           (merge.newEnd[tempname][2] - 1)) &&
          ((merge.ref_len[tempname] - ref_last) >=
           (merge.q_len[tempname] - q_last))) {
        flag = 1;
      }
    }
    if (merge.newEnd[tempname][2] > merge.newEnd[tempname][3]) //^
    {
      if (((merge.q_len[tempname] - merge.newEnd[tempname][2]) <
           (merge.newEnd[tempname][0] - 1)) &&
          ((merge.ref_len[tempname] - merge.newEnd[tempname][1]) >
           (merge.newEnd[tempname][3] - 1))) {
        flag = 1;
      }
    }
...
/////////////////////////////////////////////////////////////////////////////////////////
void sideCheckerR(
    asmMerge &merge) // checks whether the midpoint of alignment is on the right
                     // or left side of the reference
{
  string tempname;
  int rMid;
  int r_f, r_last;
...
////////////////////////////////////////////////////////////////////////////////////////
void sideChecker(asmMerge &merge) // checks whether midpoint of query is 5' or
                                  // 3' end of the reference
{
  string tempname;
  int qMid;
  int rMid;
  ...
  for (unsigned int i = 0; i < merge.r_name.size(); i++) {
    ...
    if (merge.newEnd[tempname][2] < merge.newEnd[tempname][3]) // same strand
    {
      if (merge.newEnd[tempname][2] > qMid) {
        qMidonR =
            merge.newEnd[tempname][0] - (merge.newEnd[tempname][2] - qMid);
        if (qMidonR > rMid)
        //	if((merge.newEnd[tempname][0]-1) > (merge.ref_len[tempname] -
        //merge.newEnd[tempname][1]))
        {
      ...
      if (merge.newEnd[tempname][2] < qMid) {
        qMidonR =
            merge.newEnd[tempname][0] + (qMid - merge.newEnd[tempname][2]);
        if (qMidonR > rMid) {
          merge.sideInfo[tempname] = 'R';
        }
        if (qMidonR < rMid) {
    ...
    if (merge.newEnd[tempname][2] >
        merge.newEnd[tempname][3]) // if opposite strands
    {
...
void ovlStoreCalculator(asmMerge &merge) {
  ...
  for (unsigned int i = 0; i < merge.r_name.size(); i++) {
    tempname = merge.r_name[i] + merge.q_name[i];
    merge.ovlStore[tempname] =
        ovlCalculator(merge.q_st[tempname], merge.q_end[tempname]);
    lisCalculator(merge, tempname, merge.ref_st[tempname],
                  merge.ref_end[tempname], merge.q_st[tempname],
                  merge.q_end[tempname]);
  }
}
/////////////////////////////////////////////////////////////////////////////////////
void lisCalculator(asmMerge &merge, string &tempname, vector<int> &ref_st,
                   vector<int> &ref_end, vector<int> &q_st,
                   vector<int> &q_end) {
  int ovl = 0;
    ...
    for (unsigned int j = 0; j < q_st.size(); j++) {
      if (q_st[j] < q_end[j]) // if this MUM is forward oriented
      {
        ...
        for (unsigned k = j; k < q_st.size(); k++) {
          // if((q_st[k] > (storedQend - 500)) &&
          // (abs(q_st[k]-storedQend)<merge.q_len[tempname]) && (ref_st[k] >
          // (storedRefEnd - 500))) //500bp overlap allowed
          if ((q_st[k] > storedQst) &&
              (abs(q_st[k] - storedQst) < merge.q_len[tempname]) &&
              (ref_st[k] > storedRefSt)) {
  ...
  sort(v.begin(), v.end());
  ovl = v[v.size() - 1];
  merge.newEnd[tempname].push_back(
      ovl2refSt[ovl]); // stores the LIS start and end for each
                       // alignment.elements:refSt,refEnd,qSt,qEnd
...
void nOvlStoreCalculator(asmMerge &merge) {
  ...
  for (unsigned int i = 0; i < merge.r_name.size(); i++) {
    ...
    {
      if (((merge.newEnd[tempname][2] - 1) > (merge.newEnd[tempname][0] - 1)) &&
          !((merge.q_len[tempname] - q_last) >
            (merge.ref_len[tempname] -
             ref_last))) // if left side of the query end is hanging but right
                         // side not
      {
        noLovl = merge.newEnd[tempname][0];
        noRovl = merge.q_len[tempname] - q_last;
        // cout<<"NV "<<tempname<<" "<<merge.newEnd[tempname][0]- noLovl<<"
        // "<<merge.newEnd[tempname][0]<<" "<<merge.newEnd[tempname][1]<<"
        // "<<(merge.newEnd[tempname][1]) + noRovl<<"
        // "<<merge.newEnd[tempname][2] - noLovl<<"
        // "<<merge.newEnd[tempname][2]<<" "<<merge.newEnd[tempname][3]<<" "<<
        // noRovl+merge.newEnd[tempname][3] <<endl;
      }
      if (((merge.q_len[tempname] - q_last) >
           (merge.ref_len[tempname] - ref_last)) &&
          !((merge.newEnd[tempname][2] - 1) >
            (merge.newEnd[tempname][0] -
             1))) // if right side of the query is hanging but left side is not
      {
    ...
    if (merge.newEnd[tempname][2] > merge.newEnd[tempname][3]) // reverse strand
    {
      ...
      if (((merge.q_len[tempname] - merge.newEnd[tempname][2]) >
           (merge.newEnd[tempname][0] - 1)) &&
          !((q_last - 1) > (merge.ref_len[tempname] - ref_last))) {
        noLovl = merge.newEnd[tempname][0];
        noRovl = q_last - 1;
        // cout<<"NV "<<tempname<<" "<<merge.newEnd[tempname][0] - noLovl<<"
        // "<<merge.newEnd[tempname][0]<<" "<<merge.newEnd[tempname][1]<<"
        // "<<merge.newEnd[tempname][1]+noRovl<<" "<<merge.newEnd[tempname][2] +
        // noLovl<<" "<<merge.newEnd[tempname][2]<<"
        // "<<merge.newEnd[tempname][3]<<" "<<merge.newEnd[tempname][3]
        // -noRovl<<endl;
      }

      if (((q_last - 1) > (merge.ref_len[tempname] - ref_last)) &&
          !((merge.q_len[tempname] - merge.newEnd[tempname][2]) >
            (merge.newEnd[tempname][0] - 1))) {
        noRovl = merge.ref_len[tempname] - ref_last;
...
void writeSummary(asmMerge &merge, string &prefix) {
  ...
  fout.open(fileName);
  fout << "REF" << '\t' << "QUERY" << '\t' << "REF_START" << '\t' << "REF_END"
       << '\t' << "Q_START" << '\t' << "Q_END" << '\t' << "ORIENTATION" << '\t'
       << "INNIE(1/0)" << '\t' << "OVERLAP_LEN" << '\t' << "OVERLAP_PROP"
       << '\t' << "NO_OVERLAP_AT_ENDS" << '\t' << "OVERHANG" << endl;
  for (unsigned int i = 0; i < merge.r_name.size(); i++) {
    tempname = merge.r_name[i] + merge.q_name[i];
    ...
    fout << merge.r_name[i] << '\t' << merge.q_name[i] << '\t'
         << merge.ref_st[tempname][0] << '\t'
         << merge.ref_end[tempname][lastElemRef] << '\t'
         << merge.q_st[tempname][0] << '\t' << merge.q_end[tempname][lastElemQ]
         << '\t' << merge.sideInfo[tempname] << '\t' << merge.innie[tempname]
         << '\t' << merge.ovlStore[tempname] << '\t' << ovl << '\t'
         << merge.nOvlStore[tempname] << '\t' << merge.ovrHangQ[tempname]
         << endl;
  }
  fout.close();
}
  ...
  for (unsigned int i = 0; i < merge.r_name.size(); i++) {
    ...
    if (merge.newEnd[tempname][2] < merge.newEnd[tempname][3]) {
      if ((!(((ref_f - 1) < (q_f - 1)) &&
             ((merge.ref_len[tempname] - ref_last) <
              (merge.q_len[tempname] - q_last)))) &&
          (merge.innie[tempname] != 1)) // when ref or query not innie
      {
        Dist = abs((q_f - q_last)) + merge.nOvlStore[tempname];
        ovrhangQ = merge.q_len[tempname] -
                   Dist; // this would be zero for innie (overhangRef =
                         // merge.ref_len[tempname] - Dist)
        ovrhangR = merge.ref_len[tempname] - Dist;
        if (ovrhangQ == -1) // this is to fix edge cases when alignment edges
                            // coincide with sequence ends
        {
          ovrhangQ = 0;
        }
        if (ovrhangR == -1) {
      ...
      if (((ref_f - 1) < (q_f - 1)) &&
          ((merge.ref_len[tempname] - ref_last) <
           (merge.q_len[tempname] -
            q_last))) // when ref is innie but query is not
      {
        ovrhangQ = -1;
        ...
        merge.ovrHangQR[tempname].push_back(
            ovrhangLTq); // these maps are populated only when these alignments
                         // occur
    ...
    if (merge.newEnd[tempname][2] > merge.newEnd[tempname][3]) {
      if ((!(((ref_f - 1) < (merge.q_len[tempname] - q_f)) &&
             ((merge.ref_len[tempname] - ref_last) < (q_last - 1)))) &&
          (merge.innie[tempname] != 1)) {
        Dist = abs((q_f - q_last)) + merge.nOvlStore[tempname];
        ovrhangQ = merge.q_len[tempname] -
                   Dist; // this would be zero for innie (ovrhangRef =
                         // merge.ref_len[tempname] -Dist)
        ovrhangR = merge.ref_len[tempname] - Dist;
        if (ovrhangQ == -1) // this is to fix edge cases when alignment edges
                            // coincides with sequence ends
        {
          ovrhangQ = 0;
        }
        if (ovrhangR == -1) {
      ...
      if (((ref_f - 1) < (merge.q_len[tempname] - q_f)) &&
          ((merge.ref_len[tempname] - ref_last) <
           (q_last - 1))) // when ref is innie but query is not
      {
        ovrhangQ = -1;
        ovrhangR = -2; // changed from 0 to -2
        ...
        merge.ovrHangQR[tempname].push_back(
            ovrhangLTq); // these maps get populated only when these alignments
                         // occur
        merge.ovrHangQR[tempname].push_back(ovrhangRTq);
      }
      if (merge.innie[tempname] == 1) {
    ...
    merge.ovrHangQ[tempname] =
        ovrhangQ; // this should be ovrHangQ and add ovrHangRef
...
void fillAnchor(asmMerge &merge, asmMerge &merge1, double propAnchor,
  ...
  for (unsigned int i = 0; i < merge.r_name.size(); i++) {
    ...
    if ((prop > propAnchor) &&
        (merge.ref_len[tempname] > merge.q_len[tempname]) &&
        (merge.ref_len[tempname] > length)) {
      merge.anchor[tempname].push_back(merge.r_name[i]);
      merge.anchor[tempname].push_back(merge.q_name[i]);
      if (merge.innie[tempname] == 1) {
        fillToRemove(
            merge,
            merge.q_name[i]); // here add only those for which query is an innie
      }
    }
    if ((prop > propCutoff) && (merge.ovlStore[tempname] > absLenCutoff)) {
      merge1.r_name.push_back(
          merge.r_name[i]); // populating the new asmMerge names vector
...
void trimSeq(asmMerge &merge, fastaSeq &hyb) {
  ...
  for (unsigned int i = 0; i < merge.r_name.size(); i++) {
    tempname = merge.r_name[i] + merge.q_name[i];
    if ((merge.innie[tempname] == 1) &&
        (find(qToTrim.begin(), qToTrim.end(), merge.q_name[i]) ==
         qToTrim.end())) {
...
//////////////////////////////////////////////////////////////////////////
void fillSeq(fastaSeq &fasta,
             ifstream &fin) // overloaded function. will be templated
{
  string str, str1;
...
void writeAnchorSummary(asmMerge &merge, string &prefix) {
  ...
  fout.open(fileName);
  string tempname, str;
  fout << "REF_NAME" << '\t' << "Q_NAME" << '\t' << "REF_LENGTH" << '\t'
       << "Q_LENGTH" << '\t' << "REF-ST" << '\t' << "REF-END" << '\t' << "Q-ST"
       << '\t' << "Q-END" << endl;
  for (map<string, vector<string>>::iterator it = merge.anchor.begin();
       it != merge.anchor.end(); it++) {
    tempname = it->first;
    merge.cAnchor[merge.anchor[tempname][0]].push_back(
        merge.anchor[tempname][1]);
    fout << merge.anchor[tempname][0] << '\t' << merge.anchor[tempname][1]
         << '\t' << merge.ref_len[tempname] << '\t' << merge.q_len[tempname]
         << '\t' << merge.ref_st[tempname][0] << '\t'
         << merge.ref_end[tempname][merge.ref_end[tempname].size() - 1] << '\t'
         << merge.q_st[tempname][0] << '\t'
         << merge.q_end[tempname][merge.q_end[tempname].size() - 1] << endl;
  }
  fout.close();
}
///////////////////////////////////////////////////////////////////
vector<string>
vfind(string tempname, vector<string> &temp_rname, vector<string> &temp_qname,
      asmMerge &merge, string &guruRef,
      double propCutoff) // use tempvectors for names for searching. temp_rname
                         // is where tempname is. temp_qname is where the
                         // alignment partner of r_name is.
{
  unsigned int i = 0;
  vector<string> v1;
  ...
  while (i < temp_rname.size()) {
    if (temp_rname[i] == tempname) {
      if (find(merge.q_name.begin(), merge.q_name.end(), tempname) !=
          merge.q_name.end()) // if tempname is a query
      {
        discAnchor(tempname, merge, guruRef, propCutoff);
      }

      v1.push_back(
          temp_qname[i]); // may be add the condition that only overlaps above a
                          // certain point makes it into the list

      storeRef = temp_rname[i]; // storing them before erasing, in case I have
                                // to restore them
      storeQ = temp_qname[i];
      temp_rname[i] = ""; // akin to erasing the element. actual erasing is more
                          // time consuming.
      temp_qname[i] = "";
      if (merge.cAnchor.find(storeRef) !=
          merge.cAnchor.end()) // if the reference is a contig anchor
      {
        C = 'Y';
      }
      if ((find(temp_rname.begin(), temp_rname.end(), storeRef) ==
           temp_rname.end()) &&
          (merge.innie[storeRef + storeQ] == 1) &&
          (C == 'Y')) // if the ref occurs only once and query is an innie, C
                      // means whether it is the anchor contig
      {
        if (find(temp_qname.begin(), temp_qname.end(), storeQ) !=
            temp_qname.end()) // if query occurs more than once
        {
          temp_qname[i] = storeQ;
  ...
  for (unsigned int i = 0; i < seq.size(); i++) {
    ...
    if (RorQ == 'R') // if looking for reference overhang length
    {
      ...
      if ((merge.strandInfo[str] == merge.strandInfo[prevElem]) &&
          (merge.ovrHangR[str] !=
           -1)) // if the references for the query are on the same strand
      {
        //	if(merge.sideInfo[str] != merge.sideInfo[prevElem]) // they
        //ought to be different
        if ((merge.sideInfoQ[str] != sideQ) ||
            (merge.sideInfo[str] != merge.sideInfo[prevElem])) {
          size = merge.ovrHangR[str];
        }
      }
      if ((merge.strandInfo[str] != merge.strandInfo[prevElem]) &&
          (merge.ovrHangR[str] != -1)) {
        // if(merge.sideInfo[str] == merge.sideInfo[prevElem]) // they will be
        // same if the two references are on different strands
        if ((merge.sideInfoQ[str] != sideQ) ||
            (merge.sideInfo[str] == merge.sideInfo[prevElem])) {
          size = merge.ovrHangR[str];
        }
  ...
  for (unsigned int i = 0; i < seq.size(); i++) {
    ...
    if (RorQ == 'R') // if looking for reference overhang length
    {
      if (merge.ovrHangR[str] == -1) // if query is innie
      {
        ...
        if ((merge.strandInfo[str] != merge.strandInfo[prevElem]) &&
            (merge.sideInfo[str] == 'L')) {
          size = merge.ovrHangQR[str][0]; // if the previous was right, and this
                                          // is left, take left overhang
        }
        if ((merge.strandInfo[str] != merge.strandInfo[prevElem]) &&
            (merge.sideInfo[str] == 'R')) {
      ...
      {
        // if(str == "tig00000194
        // Backbone_143/0_695345"){cout<<merge.strandInfo["tig00000194
        // Backbone_143/0_695345"]<<"\t"<<merge.strandInfo[str]<<"\t"<<merge.sideInfoQ[str]<<endl;}
        if ((merge.sideInfoQ[str] != sideQ) ||
            (merge.sideInfo[str] != merge.sideInfo[prevElem]))
      ...
      if ((merge.strandInfo[str] != merge.strandInfo[prevElem]) &&
          (merge.ovrHangR[str] != -1)) {
        // if(str == "tig00000194
        // Backbone_143/0_695345"){cout<<merge.strandInfo[str]<<"\t"<<merge.sideInfoQ[str]<<endl;}
        if ((merge.sideInfoQ[str] != sideQ) ||
            (merge.sideInfo[str] == merge.sideInfo[prevElem]))
        // if(merge.sideInfo[str] == merge.sideInfo[prevElem])
        {
...
void findChain(asmMerge &merge, asmMerge &merge1, fastaSeq &pbOnly,
  ...
  string ref, refForSideL, refForSideR, prevElem,
      guruR; // guruR stores the name of the anchor contig
  for (map<string, vector<string>>::iterator it = merge.cAnchor.begin();
       it != merge.cAnchor.end(); it++) {
    ...
    if (merge.cAnchor[tempname].size() ==
        0) // anchors which are partners of previously used query chain elements
    {
      lQseq = "";
      rQseq = "";
    }
    while ((rQseq != "") | (rRseq != "") | (lQseq != "") | (lRseq != "")) {
      ...
      if (lQseq != "") {
        ...
        seqs = vfind(lQseq, temp_qname, temp_rname, merge, guruR,
                     propCutoff); // temp_qname is where lQseq should be
                                  // present. temp_rname is where the references
                                  // will be searched from

        if (!seqs.empty()) // check that empty returns null when vfind does not
                           // resturn an argument
        {
          if (ref != "") // if tempname is currently the reference
          {
            prevElem = ref + lQseq;
            tlRseq = longestLeft(lQseq, seqs, merge, 'R',
                                 merge.sideInfoQ[tempname + lQseq], prevElem);
            prevElem = "";
          }
          if (ref == "") // if lRseq was the previous reference
          {
            ...
            tlRseq = longestLeft(lQseq, seqs, merge, 'R',
                                 merge.sideInfoQ[refForSideL + lQseq],
                                 prevElem); // tlRseq inside the parentheses are
      ...
      if (lRseq != "") {
        ...
        fillToRemove(merge, seqs);
        if (!seqs.empty()) {
          if (merge.sideInfo[lRseq + tlQseq] ==
              'L') // if the previous alignment was on the left side of the
                   // reference
          {
            lQseq = longestRt(lRseq, seqs, merge, 'Q', 'N',
                              prevElem); //'N' because this one does not require
                                         //to check for query side
          }
          if (merge.sideInfo[lRseq + tlQseq] == 'R') {
            lQseq = longestLeft(lRseq, seqs, merge, 'Q', 'N',
                                prevElem); //'N' because this one does not
                                           //require to check for query side
          }
        }
        refForSideL = lRseq;
        lRseq = "";
      ...
      if (rQseq != "") {
        ...
        if (!seqs.empty()) {
          if (ref != "") // because rRseq is empty when tempname was the
                         // previous reference
          {
            prevElem = ref + rQseq;
            trRseq = longestRt(rQseq, seqs, merge, 'R',
                               merge.sideInfoQ[tempname + rQseq], prevElem);
            prevElem = "";
          }
          if (ref == "") {
            prevElem = refForSideR + rQseq;
            trRseq = longestRt(rQseq, seqs, merge, 'R',
                               merge.sideInfoQ[refForSideR + rQseq],
                               prevElem); // refForSideR is the previous
      ...
      if (rRseq != "") {
        ...
        fillToRemove(merge, seqs);
        if (!seqs.empty()) {
          if (merge.sideInfo[rRseq + trQseq] ==
              'R') // if previous alignment was on the right side of the
                   // reference
          {
            rQseq = longestLeft(
                rRseq, seqs, merge, 'Q', 'N',
                prevElem); // again N because no need to check for query side
          }
          if (merge.sideInfo[rRseq + trQseq] ==
              'L') // if previous alignment was on the right side of the
                   // reference
          {
            rQseq = longestRt(rRseq, seqs, merge, 'Q', 'N',
                              prevElem); // again N because no need to check for
                                         // query side NEW
          }
        }
        refForSideR = rRseq;
  ...
  for (unsigned int i = 0; i < refSeqToAdd.size(); i++) {
    if ((find(refSeqToRemove.begin(), refSeqToRemove.end(), refSeqToAdd[i]) ==
         refSeqToRemove.end()) &&
        merge.innie[refSeqToAdd[i]] != 1) // ref is not innie
    // if (find(refSeqToRemove.begin(),refSeqToRemove.end(),refSeqToAdd[i]) ==
    // refSeqToRemove.end())
    {
...
void createMseq(
    asmMerge &merge,
    asmMerge &merge1) // this function creates the list of contig names (lseq)
                      // and the list of their corresponding index
{

  string name, tempname;

  for (map<string, vector<string>>::iterator it = merge.rseq.begin();
       it != merge.rseq.end(); it++) {
    name = it->first;
    merge1.lseq[name].push_back(
        name); // add the anchor reference as the first element
    for (unsigned int i = 0; i < merge.rseq[name].size(); i++) {
      ...
      if (i % 2 == 0 && i > 0) {
        tempname = merge.rseq[name][i - 1] +
                   merge.rseq[name][i]; // ith element is query
      }
      merge1.rseq[name].push_back(tempname); // fill up rseq with index names
      merge1.lseq[name].push_back(
          merge.rseq[name][i]); // fill up lseq with contig names
    }
  }

  for (map<string, vector<string>>::iterator it = merge.lseq.begin();
       it != merge.lseq.end(); it++) {
    name = it->first;
    for (unsigned int j = 0; j < merge.lseq[name].size(); j++) {
      ...
      if (j % 2 == 1 && j > 0) {
        tempname = merge.lseq[name][j] +
                   merge.lseq[name][j - 1]; // jth element is a reference
      }
      if (j % 2 == 0 && j > 0) {
        tempname = merge.lseq[name][j - 1] +
                   merge.lseq[name][j]; // j-1 th element is a reference
      }
      if (merge.rseq[name].size() != 0) {
        merge1.rseq[name].insert(merge1.rseq[name].begin(),
                                 tempname); // insert the left contig indexes
        merge1.lseq[name].insert(
            merge1.lseq[name].begin(),
            merge.lseq[name][j]); // insert the left contigs
      }
      if (merge.rseq[name].size() == 0) {
        merge1.rseq[name].insert(merge1.rseq[name].begin(), tempname);
        if (j == 0) {
          merge1.lseq[name].insert(merge1.lseq[name].begin(), name);
          merge1.lseq[name].insert(merge1.lseq[name].begin(),
                                   merge.lseq[name][j]);
        }
        if (j > 0) {
          merge1.lseq[name].insert(merge1.lseq[name].begin(),
...
void fillOri(asmMerge &merge, asmMerge &merge1) {
  ...
  for (map<string, vector<string>>::iterator it = merge1.lseq.begin();
    ...
    for (unsigned int i = 0; i < merge1.lseq[name].size(); i++) {
      ...
      if (i == 0 && (find(merge.q_name.begin(), merge.q_name.end(),
                          merge1.lseq[name][i]) !=
                     merge.q_name.end())) // if first element is query
      {
        if (merge.newEnd[merge1.rseq[name][i]][2] >
            merge.newEnd[merge1.rseq[name][i]][3]) {
          merge.Ori[name].push_back(
              1); // starting query will not be changed, the next reference will
      ...
      if (find(merge.r_name.begin(), merge.r_name.end(),
               merge1.lseq[name][i]) !=
          merge.r_name.end()) // if it is reference
      {
        if (i == 0) {
          merge.Ori[name].push_back(prev); // prev = prev *1 because reference
                                           // follows orientation of the query
        }

        if (i > 0 && (merge.newEnd[merge1.rseq[name][i - 1]][2] <
                      merge.newEnd[merge1.rseq[name][i - 1]][3])) {
          merge.Ori[name].push_back(prev);
        }
        if (i > 0 && (merge.newEnd[merge1.rseq[name][i - 1]][2] >
                      merge.newEnd[merge1.rseq[name][i - 1]][3])) {
          if ((i == 1) &&
              (merge.Ori[name][i - 1] !=
               -1)) // previous alignment was reverse but was recorded as 1
          {
            // prev = prev * (-1);
            merge.Ori[name].push_back(prev);
          } else {
      ...
      if ((i > 0) && (find(merge.q_name.begin(), merge.q_name.end(),
                           merge1.lseq[name][i]) != merge.q_name.end())) {
        if (merge.newEnd[merge1.rseq[name][i - 1]][2] >
            merge.newEnd[merge1.rseq[name][i - 1]][3]) {
          prev = prev * (-1);
          merge.Ori[name].push_back(prev);
        }
...
void removeSeq(asmMerge &merge, fastaSeq &hyb, fastaSeq &merged) {
  string seqname;
  for (map<string, string>::iterator it = hyb.seq.begin(); it != hyb.seq.end();
       it++) {
    seqname = it->first;
...
void writeMerged(fastaSeq &merged, ofstream &fout) {
  ...
  for (map<string, string>::iterator it = merged.seq.begin();
       it != merged.seq.end(); it++) {
    seqname = it->first;
    index = seqname;
...
void ctgJoiner(asmMerge &merge, asmMerge &merge1, fastaSeq &hybrid,
               fastaSeq &pbOnly, fastaSeq &merged) {
  string name, subseq, subseqR, indexL1, indexL2, seqHolder;
  vector<int> v1;
  int q1_f, q1_last, q2_f, q2_last, r1_f, r1_last, r2_f, r2_last, tempRef_st,
      cheatF, cheatR;
  int begin_insrt = 1;
  bool skip = false;
  for (map<string, vector<string>>::iterator it = merge1.lseq.begin();
       it != merge1.lseq.end();
       it++) // merge1.lseq is container for the contig names
  {
    name = it->first;
    begin_insrt = 1;
    tempRef_st = 0;
    for (unsigned int i = 0; i < merge1.lseq[name].size(); i++) {
                     ...
                     merge.q_name.end())) // if first element is query
      {
        indexL2 =
            merge1
                .rseq[name][i]; // index name coresponding to the query element
        ...
        if (merge1.lseq[name][0] ==
            merge1.lseq[name][merge1.lseq[name].size() -
                              1]) // if the first and last are same
        {
      ...
      if (i == 0 && (find(merge.r_name.begin(), merge.r_name.end(),
                          merge1.lseq[name][i]) !=
                     merge.r_name.end())) // if first element is reference
      {
        ...
        if (merge1.lseq[name][i] != name) {
          ...
          if ((merge.Ori[name][i] == 1) && (merge.Ori[name][i + 1] == -1) &&
              (merge.sideInfoQ[indexL2] == 'R'))

          {
            subseq = pbOnly.seq[merge1.lseq[name][i]].substr(0, r2_last);
          }
          if ((merge.Ori[name][i] == 1) && (merge.Ori[name][i + 1] == -1) &&
              (merge.sideInfoQ[indexL2] == 'U') &&
              (merge.sideInfo[indexL2] == 'R')) {
            subseq = pbOnly.seq[merge1.lseq[name][i]].substr(0, r2_last);
          }
          ...
          if ((merge.Ori[name][i] == 1) && (merge.Ori[name][i + 1] == -1) &&
              (merge.sideInfoQ[indexL2] == 'L')) {
            subseq = pbOnly.seq[merge1.lseq[name][i]].substr(
                r2_f, (merge.ref_len[indexL2] - r2_f));
            begin_insrt = -1;
          }
          if ((merge.Ori[name][i] == 1) && (merge.Ori[name][i + 1] == -1) &&
              (merge.sideInfoQ[indexL2] == 'U') &&
              (merge.sideInfo[indexL2] == 'L')) {
            subseq = pbOnly.seq[merge1.lseq[name][i]].substr(
                r2_f, (merge.ref_len[indexL2] - r2_f));
            begin_insrt = -1;
          }
      ...
      if (i > 0 && (find(merge.q_name.begin(), merge.q_name.end(),
                         merge1.lseq[name][i]) !=
                    merge.q_name.end())) // if the element is query
      {
        indexL1 = merge1.rseq[name][i - 1];
        q1_f = merge.newEnd[indexL1][2];
        q1_last = merge.newEnd[indexL1][3];
        if ((merge.Ori[name][i - 1] == 1) &&
            merge.sideInfo[merge1.rseq[name][i - 1]] == 'L') {
          begin_insrt = -1;
        }
        if (i != (merge1.lseq[name].size() - 1)) // if it isn't the last element
        {
          ...
          q2_f = merge.newEnd[indexL2][2];
          q2_last = merge.newEnd[indexL2][3];
          if (chkOvl(q1_f, q1_last, q2_f, q2_last) ==
              0) // alignments are non-overlapping
          {
            v1 = minD(q1_f, q1_last, q2_f, q2_last);
            cheatF = min(v1[0], v1[1]);
            cheatR = max(v1[0], v1[1]);
            subseq = hybrid.seq[merge1.lseq[name][i]].substr(cheatF,
                                                             (cheatR - cheatF));
          }
          if (chkOvl(q1_f, q1_last, q2_f, q2_last) !=
              0) // if alignments are overlapping
          {
            tempRef_st = mapQonRef(indexL1, indexL2, merge);
            subseq = ""; // may be add a single base if this causes segfault for
                         // revcom
          }
        }
        if (i == (merge1.lseq[name].size() - 1)) // if this is the last element
        {
          ...
          if (merge.newEnd[merge1.rseq[name][i - 1]][2] >
              merge
                  .newEnd[merge1.rseq[name][i - 1]][3]) // if reverse complement
          {
            subseq = hybrid.seq[merge1.lseq[name][i]].substr(0, q1_last);

            if ((i > 1) && (merge1.lseq[name][i] ==
                            merge1.lseq[name][i - 2])) // if two queries for the
                                                       // reference are same
            {
              if (merge.sideInfoQ[merge1.rseq[name][i - 3]] == 'L') {
                subseq = hybrid.seq[merge1.lseq[name][i]].substr(
                    q1_f, (merge.q_len[merge1.rseq[name][i - 1]] - q1_f));
              }
              if (merge.sideInfoQ[merge1.rseq[name][i - 3]] == 'R') {
                subseq = hybrid.seq[merge1.lseq[name][i]].substr(0, q1_last);
              }
            }
            if (merge1.lseq[name][i] ==
                merge1.lseq[name]
                           [0]) // if first element and last elements are same
            {
              subseq = hybrid.seq[merge1.lseq[name][i]].substr(0, q1_last);
            }
            // if((merge.sideInfo[merge1.rseq[name][i-1]] == 'L') &&
            // !((merge1.lseq[name][i] == merge1.lseq[name][i-2]))) //if the
            // reference 5' is away from seed (TODO:bring it one step up and use
            // elseif)
            if ((merge.overHangSideQ[merge1.rseq[name][i - 1]] == 'L') &&
                (i > 1) &&
                (!(merge1.lseq[name][i] == merge1.lseq[name][i - 2]))) {
              // subseq = hybrid.seq[merge1.lseq[name][i]].substr(q1_last); //go
              // from smallest to the end of the query
              subseq = hybrid.seq[merge1.lseq[name][i]].substr(q1_f);
            }
            if ((merge.overHangSideQ[merge1.rseq[name][i - 1]] == 'R') &&
                (i > 1) &&
                (!(merge1.lseq[name][i] == merge1.lseq[name][i - 2]))) {
              subseq = hybrid.seq[merge1.lseq[name][i]].substr(0, q1_last);
            }
          ...
          if (merge.newEnd[merge1.rseq[name][i - 1]][2] <
              merge.newEnd[merge1.rseq[name][i - 1]][3]) // same strand
          {
            if (merge.sideInfo[merge1.rseq[name][i - 1]] == 'L') {
      ...
      if (i > 0 && (find(merge.r_name.begin(), merge.r_name.end(),
                         merge1.lseq[name][i]) !=
                    merge.r_name.end())) // if the element is reference
      {
        indexL1 = merge1.rseq[name][i - 1];
        ...
        if (i !=
            (merge1.lseq[name].size() - 1)) // if this is not the last element
        {
          indexL2 = merge1.rseq[name][i];
          r2_f = merge.newEnd[indexL2][0];
          r2_last = merge.newEnd[indexL2][1];
          ...
          v1 = maxD(r1_f, r1_last, r2_f,
                    r2_last); // maximum distance between coords from the two
                              // alignments
          r1_f = min(v1[0], v1[1]);
          r2_last = max(v1[0], v1[1]);

          subseq =
              pbOnly.seq[merge1.lseq[name][i]].substr(r1_f, (r2_last - r1_f));
          tempRef_st = 0; // reset tempRef_st
          if ((merge.sideInfoQ[indexL1] == 'L') &&
              (merge.sideInfoR[indexL1] == 'R') &&
              (merge.Ori[name][i - 1] == 1) && (merge.Ori[name][i] == 1)) {
            begin_insrt = -1;
          }
          if ((merge.sideInfoQ[indexL1] == 'L') &&
              (merge.sideInfoR[indexL1] == 'L') &&
              (merge.Ori[name][i - 1] == 1) && (merge.Ori[name][i] == -1)) {
            begin_insrt = -1;
          }
        }
        if (i == (merge1.lseq[name].size() - 1)) {
          ...
          if (merge.sideInfoR[indexL1] == 'L') {
            ...
            subseq = pbOnly.seq[merge1.lseq[name][i]].substr(
                r1_f,
                (merge.ref_len[indexL1] - r1_f)); // r_last is actually r1_f
          }
          //	}
          //	else
          //	{
          //		if(tempRef_st != 0)
          //		{
          //			r1_f = tempRef_st; //transfer the number so that
          //it is used in case of an overlap; 			tempRef_st = 0; //reset tempRef
          // cout<<name<<"\t"<<r1_f<<endl;
          //		}
          //	}
          // }
          if (subseq.size() == 0) // if subseq has not been modified by
                                  // condition 1 in the preceding section
          {
            subseq = pbOnly.seq[merge1.lseq[name][i]].substr(
                r1_f, (merge.ref_len[indexL1] - r1_f));
          }
        }
        if (merge.Ori[name][i] == -1) {
      ...
      subseq.clear(); // experimental. to use the zero size of subseq as a
                      // condition
      if (skip == true) {
    ...
    cout << "Merged Contig and length: " << name << "\t" << seqHolder.size()
         << endl;
    seqHolder.clear();
  }
}
...
int chkOvl(int f1, int e1, int f2, int e2) {
  int flag = 0;
  if (maxIntD(f1, e1, f2, e2) < (abs(f1 - e1) + abs(f2 - e2))) {
    flag = abs(maxIntD(f1, e1, f2, e2) -
               (abs(f1 - e1) +
                abs(f2 - e2))); // if there is an overlap measure the ovl
  }
  return flag;
}
//////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////
void sideCheckerQ(asmMerge &merge) // checks whether the midpoint of alignment
                                   // is on the right or left side of the query
{
  string tempname;
  int qMid;
  int q_f, q_last;
  ...
  for (unsigned int i = 0; i < merge.r_name.size(); i++) {
    ...
    if (q_f < q_last) {
      ...
      {
        merge.sideInfoQ[tempname] =
            merge.sideInfo[tempname]; // having 'U' was leading to missing
                                      // segments in pac to pac merging
      }
    }
...
void discAnchor(string &guruQ, asmMerge &merge, string &guruRef,
                double propCutoff) // discard anchor contig that is partner of a
                                   // query contig being used for chaining
{
  ...
  for (map<string, vector<string>>::iterator it = merge.cAnchor.begin();
    ...
    if ((merge.ovlStore[temp] > 0) && (guruRef != pastTemp) &&
        (cutoff > propCutoff) && (guruRef != it->first)) {
      merge.cAnchor[it->first].clear();
    }
  }
}
////////////////////////////////////////////////////////
void overHangSideR(asmMerge &merge) // whether overhanging query is on the 5' or
                                    // 3' end of the reference
{
  string tempname;
  int LnOverlap = 0; // part of query at the 5'side of the reference that does
                     // not align to reference
  int RnOverlap = 0; // part of the query at the 3' side of the reference that
  ...
  for (unsigned int i = 0; i < merge.r_name.size(); i++) {
    ...
    if (q_end < q_st) // if the query is on the other stand
    {
      ...
      if (LqMaponRef < 0) {
        merge.overHangSideQ[tempname] =
            'L'; // overhang is on the left side of the reference
      }
      if (RqMaponRef > merge.ref_len[tempname]) {
        merge.overHangSideQ[tempname] =
            'R'; // overhang is on the right side of the reference
      }
    }
    if (q_end > q_st) {
      LnOverlap = q_st - 1;
...
void splitHaplo(asmMerge &merge, fastaSeq &hybrid) {
  ...
  for (map<string, string>::iterator it = hybrid.seq.begin();
       it != hybrid.seq.end(); it++) {
    hybname = it->first;
    if (find(merge.q_name.begin(), merge.q_name.end(), hybname) ==
        merge.q_name.end()) // if the fasta seq is not present in the delta file
    {
      fout << ">" << hybname << endl;
      fout << it->second << endl;
      hybrid.seq.erase(it);
