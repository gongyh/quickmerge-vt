...
void checkAln(asmMerge &merge, asmMerge &merge1) {
  ...
  vector<string> vs;
  double co = 0.0;
  for (map<string, vector<string>>::iterator it = merge1.rseq.begin();
       it != merge1.rseq.end(); it++) {
    name = it->first;
    for (unsigned int i = 0; i < merge1.rseq[name].size(); i++) {
      ...
      // cout<<name<<" "<<refName<<" "<<qName<<"\t"<<rL<<"\t"<<rR<<"\t"<<"merge
      // r seq has "<<merge.rseq[it->first].size()<<" "<<it->first<<endl;
      if ((rL > 200) || (rR > 200)) {
        //	cout<<name<<" "<<refName<<"
        //"<<qName<<"\t"<<rL<<"\t"<<rR<<"\t"<<"merge r seq has
        //"<<merge.rseq[it->first].size()<<" "<<it->first<<endl;
        for (unsigned int k = 0; k < merge1.q_name.size(); k++) {
          ...
          if (merge.newEnd[temp].size() != 0) // alignment exists
          {
            ...
            // cout<<name<<"\t"<<temp<<endl;
            if ((!(re < rL1) && !(re > rL2)) &&
                (find(vs.begin(), vs.end(), temp) == vs.end())) {
              ovlLen = min(rL2, re) - max(rL1, rs);
              co = double(ovlLen) / double(rL);
              if (co > 0.9) {
                //								cout<<"R
                //L "<<temp<<" "<<merge.newEnd[temp][0]<<"
                //"<<merge.newEnd[temp][1]<<" "<<rL1<<" "<<rL2<<" "<<rL<<"
                //"<<ovlLen<<" "<<merge.innie[temp]<<" "<<co<<endl;
                vs.push_back(temp);
              }
            }
            if ((!(rs < rR1) && !(rs > rR2)) &&
              ...
              if (co > 0.9) {
                //								cout<<"R
                //R "<<temp<<" "<<merge.newEnd[temp][0]<<"
                //"<<merge.newEnd[temp][1]<<" "<<rR1<<" "<<rR2<<" "<<rR<<"
                //"<<ovlLen<<" "<<merge.innie[temp]<<" "<<co<<endl;
                vs.push_back(temp);
              }
            }
          ...
          if (merge.newEnd[temp].size() != 0) {
            ...
            if (qL1 < qL2) // ovl is forward
            {
              ...
              if ((!(qe < qL1) && !(qe > qL2)) &&
                ...
                if (co > 0.9) {
                  //									cout<<"qF
                  //L "<<temp<<" "<<merge.newEnd[temp][2]<<"
                  //"<<merge.newEnd[temp][3]<<" "<<qL1<<" "<<qL2<<" "<<rL<<"
                  //"<<ovlLen<<" "<< co<<endl;
                  vs.push_back(temp);
                }
              }
            ...
            if (qL1 > qL2) // novl is reverse but ovl is forward
            {
              ...
              if ((!(qe > qR1) && !(qe < qR2)) &&
                ...
                if (co > 0.9) {
                  //									cout<<"qR
                  //R "<<temp<<" "<<merge.newEnd[temp][2]<<"
                  //"<<merge.newEnd[temp][3]<<" "<<qR1<<" "<<qR2<<" "<<rR<<"
                  //"<<ovlLen<<" "<<co<<endl;
                  vs.push_back(temp);
                }
              }
